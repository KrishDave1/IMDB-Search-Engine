...
if choice == "1":
    ...
    def link():
        selection_class = "trending-list-rank-item-name"
        topic_title_tags = soup.find_all("span", {'class': selection_class})
        l = []
        for i in topic_title_tags:
            tmp = i.find("a")
            l.append('https://www.imdb.com' + tmp.get('href'))
        return l
    ...
    def link():
        ...
        for i in topic_title_tags:
            link = i.find('a')['href']
            link_l.append('https://www.imdb.com' + link)
        return link_l
    dict={
